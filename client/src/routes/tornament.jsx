...
export default function Tournament() {
  ...
  return <main className="editorialPage tournamentsV3">
    <PageHero eyebrow={copy("tournaments.hero.eyebrow","ATP competition")} title={<Lines>{copy("tournaments.hero.title","Put your game\non the line.")}</Lines>} text={copy("tournaments.hero.text","Organised match play for juniors and adults who want the focus, energy and growth that only competition brings.")} image={heroImage} actions={<AtpButton to="/login?redirect=%2Fu%2Ftournaments">{copy("tournaments.hero.cta","Enter a tournament")}</AtpButton>} />
    <section className="eventCalendar pagePad">
      <SectionHeading eyebrow={copy("tournaments.calendar.eyebrow","Competition calendar")} title={copy("tournaments.calendar.title","Your next match starts here.")} text={copy("tournaments.calendar.text","Choose an event that fits your level. ATP members can register from their player dashboard.")} />
    ...
    <section className="championWall pagePad">
      <SectionHeading light eyebrow={copy("tournaments.archive.eyebrow","Championship archive")} title={copy("tournaments.archive.title","Earned on court.")} text={copy("tournaments.archive.text","Every draw leaves a record. Meet the champions and finalists who raised the standard.")} />
    ...
    <section className="registrationSteps pagePad"><SectionHeading eyebrow={copy("tournaments.entry.eyebrow","How to enter")} title={copy("tournaments.entry.title","Four steps to match day.")} /><div>{[["01", copy("tournaments.entry.stepOne","Create your account")], ["02", copy("tournaments.entry.stepTwo","Choose membership")], ["03", copy("tournaments.entry.stepThree","Book your tournament")], ["04", copy("tournaments.entry.stepFour","Show up ready")]].map((step) => <article key={step[0]}><span>{step[0]}</span><h3>{step[1]}</h3></article>)}</div></section>
  </main>;
}
