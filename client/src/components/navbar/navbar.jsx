...
export default function Navbar() {
  ...
  return <header className={`navV2 ${scrolled || location.pathname !== "/" ? "solid" : ""} ${open ? "menuOpen" : ""}`}>
    <nav>
      ...
      <div className="navLinks">{links.map(link => <NavLink key={link.to} to={link.to}>{link.name}</NavLink>)}<a {...shopLinkProps('/catalog')}>Shop</a></div>
      <div className="navTools">
        <a className="navCart" {...shopLinkProps('/cart')} aria-label="Open ATP ROYALE bag"><Icon icon="solar:bag-3-linear" /></a>
    ...
    {open && <div className="navMobile">{links.map((link, index) => <NavLink key={link.to} to={link.to}><span>{String(index + 1).padStart(2, "0")}</span>{link.name}<Icon icon="solar:arrow-right-linear" /></NavLink>)}<a {...shopLinkProps('/catalog')}><span>{String(links.length + 1).padStart(2, "0")}</span>ATP ROYALE<Icon icon="solar:arrow-right-up-linear" /></a><div>{currentUser ? <Link to="/u">Open player dashboard</Link> : <><Link to="/login">Sign in</Link><Link to="/signup">Create account</Link></>}</div></div>}
  </header>;
}
