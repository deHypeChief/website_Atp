...
const groups = [
  { title: "Play", links: [["Training", "/coaching"], ["Tournaments", "/tournaments"], ["Leaderboard", "/leaderboard"], ["Community", "/community"]] },
  { title: "Explore", links: [["About ATP", "/about"], ["Gallery", "/gallery"], ["News", "/news"], ["ATP ROYALE", { shop: "/catalog" }], ["Contact", "/contact"]] },
  { title: "Player", links: [["Dashboard", "/u"], ["My coach", "/u/coach"], ["Tickets", "/u/tickets"], ["Orders", { shop: "/orders" }]] },
];
...
export default function Footer() {
  return <footer className="footerV2">
    ...
    <div className="footerLinks">
      {groups.map(group => <div key={group.title}><h3>{group.title}</h3>{group.links.map(([label, to]) => typeof to === 'object' ? <a key={to.shop} {...shopLinkProps(to.shop)}>{label}</a> : <Link key={to} to={to}>{label}</Link>)}</div>)}
